...
int main() {
    ...
    Position obstacle(0, 0);
    while (!(obstacle.x == map.colsLen()-1 && obstacle.y == map.rowsLen()-1)) {
        char temp = map.at(obstacle);

        if (!(obstacle == currPos || obstacle == updatePosition(currPos, map.at(currPos)) || map.at(obstacle) == '#')) {
        ...
        map.change(obstacle, temp);

        if (obstacle.x >= map.colsLen() - 1){
            obstacle.x = 0;
            obstacle.y += 1;
        } else {
            obstacle.x += 1;
